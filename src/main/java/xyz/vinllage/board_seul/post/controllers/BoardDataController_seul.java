...
public class BoardDataController_seul {
    ...
    // 게시글 목록 조회
    @GetMapping("/list/{bid}")
    public ResponseEntity<?> getList(@PathVariable("bid") String bid, @ModelAttribute BoardSearch_seul search) {
        try {
            search.setBid(bid);
            ...
            response.put("items", data.getItems());
            response.put("pagination", data.getPagination());
    ...
    public ResponseEntity<?> getWriteForm(@PathVariable("bid") String bid) {
        try {
            Board_seul board = configInfoService.get(bid);
            if (board == null) {
                return ResponseEntity.notFound().build();
    ...
    public ResponseEntity<?> getUpdateForm(@PathVariable("seq") Long seq, HttpSession session) {
        try {
            BoardData_seul boardData = infoService.get(seq);
            if (boardData == null) {
                return ResponseEntity.notFound().build();
